...
contract LendingPoolConfigurator {
    error LendingPoolConfigurator__OnlyPoolAdmin();
    error LendingPoolConfigurator__ReserveLiquidityNotZero();
    error LendingPoolConfigurator__AlreadyInitialized();
     ...
     *
     */
    event ReserveInitialized(address indexed _reserve, address indexed _aToken, address _interestRateStrategyAddress);
    ...
    event ReserveEnabledAsCollateral(
        address indexed _reserve, uint256 _ltv, uint256 _liquidationThreshold, uint256 _liquidationBonus
    );
    ...
    /**
     * @dev only the lending pool admin can call functions affected by this modifier
     *
     */
    modifier onlyLendingPoolManager() {
        if (poolAddressesProvider.getLendingPoolManager() != msg.sender) {
            revert LendingPoolConfigurator__OnlyPoolAdmin();
        }
        _;
    }
     ...
     *
     */
    function initReserve(address _reserve, uint8 _underlyingAssetDecimals, address _interestRateStrategyAddress)
        external
        onlyLendingPoolManager
    {
        IERC20Metadata asset = IERC20Metadata(_reserve);
        ...
        initReserveWithData(_reserve, aTokenName, aTokenSymbol, _underlyingAssetDecimals, _interestRateStrategyAddress);
    }
    ...
    function initReserveWithData(
        ...
        AToken aTokenInstance =
            new AToken(address(poolAddressesProvider), _reserve, _underlyingAssetDecimals, _aTokenName, _aTokenSymbol);
     ...
     *
     */
    function enableBorrowingOnReserve(address _reserve, bool _stableBorrowRateEnabled) external onlyLendingPoolManager {
        LendingPoolCore core = LendingPoolCore(payable(poolAddressesProvider.getLendingPoolCore()));
    ...
    function deactivateReserve(address _reserve) external onlyLendingPoolManager {
        LendingPoolCore core = LendingPoolCore(payable(poolAddressesProvider.getLendingPoolCore()));
        if (core.getReserveTotalLiquidity(_reserve) != 0) {
            revert LendingPoolConfigurator__ReserveLiquidityNotZero();
        }
     ...
     *
     */
    function setReserveLiquidationThreshold(address _reserve, uint256 _threshold) external onlyLendingPoolManager {
        LendingPoolCore core = LendingPoolCore(payable(poolAddressesProvider.getLendingPoolCore()));
     ...
     *
     */
    function setReserveLiquidationBonus(address _reserve, uint256 _bonus) external onlyLendingPoolManager {
        LendingPoolCore core = LendingPoolCore(payable(poolAddressesProvider.getLendingPoolCore()));
    ...
    /**
     * @dev retained for compatibility; the Solidity 0.8 core reads its addresses provider directly
     *
     */
    function refreshLendingPoolCoreConfiguration() external onlyLendingPoolManager {
        // The Solidity 0.8 core reads its immutable addresses provider directly, so no refresh is needed.
    }
}
